"""
specaugment.py — Mel-spectrogram feature extraction with SpecAugment augmentation.

Converts raw audio files into grayscale mel-spectrogram images using four modes:
    normal     : no augmentation
    time_mask  : random time-axis masking
    freq_mask  : random frequency-axis masking
...
from config import SAMPLE_RATE, N_MELS, TRAIN_AUDIO_DIR, TEST_AUDIO_DIR, SPEC_SAVE_DIR
...
def spec_augment(init_path, sr, mode, savepath, n_mels):
    """
    Convert all audio files in init_path to mel-spectrogram images and save them.
    ...
    for count, sample in enumerate(samples):
        ...
        image = scale_minmax(s_db, 0, 255).astype(np.uint8)
        image = 255 - np.flip(image, axis=0)

        imsave(savepath.format(count), image)
        print(f"[spec/{mode}] Saved #{count}")
...
if __name__ == '__main__':
    # Train set 
    spec_augment(TRAIN_AUDIO_DIR, SAMPLE_RATE, 'normal',    os.path.join(SPEC_SAVE_DIR, 'Train',    'image{}.png'), N_MELS)
    spec_augment(TRAIN_AUDIO_DIR, SAMPLE_RATE, 'time_mask', os.path.join(SPEC_SAVE_DIR, 'Timemask', 'image{}.png'), N_MELS)
    spec_augment(TRAIN_AUDIO_DIR, SAMPLE_RATE, 'freq_mask', os.path.join(SPEC_SAVE_DIR, 'Freqmask', 'image{}.png'), N_MELS)
    spec_augment(TRAIN_AUDIO_DIR, SAMPLE_RATE, 'time_warp', os.path.join(SPEC_SAVE_DIR, 'Timewarp', 'image{}.png'), N_MELS)

    # Test set (no augmentation)
    spec_augment(TEST_AUDIO_DIR, SAMPLE_RATE, 'normal',
                 os.path.join(SPEC_SAVE_DIR, 'Test', 'image{}.png'), N_MELS)
